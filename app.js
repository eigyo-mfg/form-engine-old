...
async function run() {
    ...
    //forでループさせる。最初は入力ページをだよというのを抜き出す。何回目の試行かも書かせる。3回目以降はやらない。今いるページの特定、ページに応じた処理の呼び出し、関数にして呼ぶ。確認だったら確認の処理をする。完了だったら結果をスプレッドに書くでループを抜ける。
    ...
    processOnInput(page).catch(console.error);
    // await browser.close();
}
